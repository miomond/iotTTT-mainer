...
function InputComment({ data }) {
  ...
  function removeLi(index) {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.delete( `http://localhost:8000/comment/comments/${index}`);
        setincomments(response.data.data);
        getcomments()
      } catch (error) {
        console.error("Error fetching data:", error); // Handle errors gracefully
    ...
    <>
      <div className="con">
        <div id="myDIV" className="header">
          <input className="inputs" onChange={changeInput} type="text" id="myInput" placeholder="Title..."/>
          <button className="bt" id="bt" onClick={commenting}> <span className="msg" id="msg"></span> Add</button>
        </div>
        <div id="myUL">
        {incomments?.length > 0 && ( incomments.map((item, index) => {
            return (
              <div key={index} className="container-comment">
                <textarea id="texter" className={item.done ? "checked" : ""} onClick={() => beDone(index)} value={item?.comment} readOnly style={{width: "560px"}}/>
                <button class="button-del-comment" onClick={() => removeLi(item?._id)} >
                  <span>Delete!</span>
                  <span>
